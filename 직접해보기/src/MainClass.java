...
public class MainClass { // RunClass라는 클래스를 생성한다.
	public static void main(String[] args) {
		Student4[] sArray = new Student4[3]; // 10명 학생의 배열 객체를 생성한다.
		
		Scanner scan = new Scanner(System.in); // 값을 입력받기 위한 Scanner 객체를 생성한다. (scan은 Scanner객체를 참조하는 변수이다.)
		
		for (int i = 0; i < sArray.length; i++) { // 국어, 영어, 수학 점수를 10번 반복하여 출력하게 한다.
			
			sArray[i] = new Student4(); // 클래스 역할을 하는 객체를 sArray[i]로 선언한다.
